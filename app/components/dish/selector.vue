  ...
  <div class="card">
    ...
    <div class="result-card">
      ...
      <div v-if="selectedDishes.length > 0" class="w-full text-left p-2 space-y-3">
        <div v-for="(dish, index) in selectedDishes" :key="dish.name + index" class="locked-in-item">
          <p class="text-2xl font-bold text-red-800">{{ dish.name }}</p>
          <div class="flex justify-between items-baseline">
            <p class="text-sm text-gray-500 bg-gray-200 inline-block px-2 py-0.5 rounded-full">{{ dish.restaurantName }}</p>
            <p class="text-lg font-bold text-green-600">{{ dish.price }}</p>
          </div>
        </div>
      </div>
    ...
    <div class="flex gap-4 items-center justify-center flex-wrap mt-4">
      ...
      <div class="flex items-center gap-2">
        ...
        <input type="number" id="count" v-model.number="countToRecommend" min="1" class="input w-20 text-center">
      </div>
      <button
